...
OUTPUT_DIR_PATH = "./rawNet2/best_model_min_dcf_0.5008/stats_all_run"
...
def save_all_plots(rows: list[TimingRow], output_dir: Path) -> list[Path]:
    ...
    for split_type in (1, 2):
        for metric_name, y_label, file_stem in PLOT_SPECS:
            plot_paths.append(
                plot_metric(rows, cumulative_minutes, split_type, metric_name, y_label, file_stem, output_dir)
            )
    return plot_paths
