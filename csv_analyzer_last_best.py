...
CSV_INPUT_PATH = "/Users/stanyin/Documents/CsvAnalyzer/rawNet2/best_model_min_dcf_0.5008/timing.csv"
OUTPUT_DIR_PATH = "./rawNet2/best_model_min_dcf_0.5008/stats_last_best"
...
def plot_metric(
    ...
    filtered = [
        ...
        if row.split_type == split_type and row.run_num <= max_run_num
    ]
    filtered.sort(key=lambda pair: pair[0])

    if not filtered:
        raise ValueError(
            f"No {SPLIT_NAMES[split_type]} rows found with run_num <= {max_run_num}."
        )
...
def save_all_plots(rows: list[TimingRow], output_dir: Path) -> list[Path]:
    max_run_num = get_last_best_dev_run_num(rows)
    cumulative_minutes = compute_cumulative_minutes(rows)
